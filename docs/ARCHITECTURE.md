...
状态：Candidate，A0 文档提案；Decision Authority：Owner。本文是 [需求](REQUIREMENTS.md) 的结构设计；操作字段以 [接口提案](INTERFACE_PROFILE.md) 为准，执行顺序见 [实施计划](IMPLEMENTATION_PLAN.md)。当前无实现证据。
...
首次创建库只接受新目标路径；打开既有库严格检查本地格式版本，不初始化覆盖、不隐式升级。文件位于本地可靠文件系统。初选 SQLite rollback journal（DELETE）与 `synchronous=FULL`，启用外键；所有新记录、引用索引、BLOB 和幂等结果进入同一事务。完整性不能仅依赖数据库外键，还需应用级全局约束验证。
...
尚待验证：上限附近的内存峰值、SQLite/OS I/O 故障的结果分类、实际序列化的跨入口一致性、未来消费者的适配成本。A1 目标是正确性和可定位错误，不承诺吞吐量或真实掉电耐久性。支持范围扩大前必须补证，不能用接口设计本身作完成证明。
