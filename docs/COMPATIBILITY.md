...
以上是 A1 0.1.0a1 历史证据；A2 当前修订 D9 `928b3e0338e7460b9916bf27fc832ce5697b1b7a`，tree `e77d8b1e2f99f2bb05473503a63ef19a6df9716c`。D1–D8 历史与 D9 来源、构建和日志见 [A2_VALIDATION](A2_VALIDATION.md)，不跨提交挪用结果。

本轮环境 Linux x86_64 / Python 3.11.16、3.12.14 / SQLite 3.53.1；235 个文件按固定 tree 核验，17 个运行模块的源码、wheel、安装字节一致。复用已有 build venv 及 system-site-packages 构建工具；Python 3.11 runtime venv 独立新建，3.12 复用此前环境。安装态从源码目录外以 -I 运行，测试/工具来自固定 D9；本地是核验后的文件集，并非 Git checkout。

两版本源码各 442 项发现、422 项通过、20 项跳过；安装态 A2 276 项发现、265 项通过、11 项跳过。12 条固定源码构建、安装、测试、编译和环境命令全部 exit 0。两版本 compileall、A1 安装态闭环、生产 API/CLI 对实际 overlay 的拒绝均通过。20 项资源专项本轮未重跑：A1 9 项保留 D2 历史，A2 11 项保留 D6 历史；安装态跳过的是同一批 A2 专项。历史结果不记作 D9 执行，不相加不同版本/安装态的重复用例。

D9 为验收工具补齐已有 A2 JSON 编码与预算规则，并使解析失败不再覆盖原错误或丢失命令记录；新增 6 项回归，含深度 8、1,024 节点、64 KiB 的 JSON 层正例，见 [本轮证据](evidence/2026-09-21-a2-review8/README.md)。产品运行模块与 D8 相同，工具不在 wheel 内，复跑须同时取得 D9 工具。D8 子进程清理、此前配额分类、源保护、最终核验、输出通道与目录绑定回归继续通过。未执行被阻断的锁失效重演；管理性稳定前置等边界不变。
